...
impl TypeChecker {
    pub fn new() -> Self {
        ...
        env.insert(
            ...
            Scheme {
                vars: vec!["T".into()],
                typ: Type::Arrow(
                    vec![("xs".into(), Type::List(Box::new(Type::Var("T".into()))))],
                    Box::new(Type::I64),
                    Box::new(Type::Row(vec![], None)),
                ),
            },
        ...
        env.insert(
            ...
            Scheme {
                ...
                typ: Type::Arrow(
                    vec![(
                        "arr".into(),
                        Type::Borrow(Box::new(Type::Array(Box::new(Type::Var("T".into()))))),
                    )],
                    Box::new(Type::I64),
                    Box::new(Type::Row(vec![], None)),
                ),
        ...
        for def in &program.definitions {
            match &def.node {
                TopLevel::Import(import) => {
                    if !import.is_external {
                        ...
                        for def in &p.definitions {
                            match &def.node {
                                TopLevel::Function(f) if f.is_public => {
                                    public_env.insert(
                                        f.name.clone(),
                                        sub_checker.generalize_top_level(f),
                                    );
                                }
                                TopLevel::ExternalFn(f) if f.is_public => {
                                    public_env.insert(
                                        f.name.clone(),
                                        Scheme {
                                            vars: vec![],
                ...
                TopLevel::Function(func) => {
                    self.env
                        .insert(func.name.clone(), self.generalize_top_level(func));
                }
                TopLevel::ExternalFn(func) => {
                    self.env.insert(
                        func.name.clone(),
                        Scheme {
                            vars: vec![],
    ...
    fn convert_user_defined_to_var(&self, typ: &Type, vars: &HashSet<String>) -> Type {
        match typ {
            ...
            Type::Result(o, e) => Type::Result(
                Box::new(self.convert_user_defined_to_var(o, vars)),
                Box::new(self.convert_user_defined_to_var(e, vars)),
            ),
        ...
        match &e.node {
            ...
            Expr::Constructor(name, args) => {
                if name == "Ok" && args.len() == 1 {
                    let (s, t) = self.infer(env, &args[0], er, ee)?;
            ...
            Expr::List(exprs) => {
                ...
                if contains_ref(&final_elem_type) {
                    return Err(TypeError {
                        message: "List cannot contain References".into(),
                        span: e.span.clone(),
                    });
                }
            ...
            Expr::Match { target, cases } => {
                ...
                let mut s = s1;
                self.check_exhaustiveness(&apply_subst_type(&s, &tt), cases)
                    .map_err(|m| TypeError {
                        message: m,
                        span: e.span.clone(),
                    })?;
        ...
        match &p.node {
            ...
            Pattern::Constructor(n, pats) => {
                if n == "Ok" && pats.len() == 1 {
                    ...
                    let s = self
                        .unify(tt, &Type::Result(Box::new(tok.clone()), Box::new(terr)))
                        .map_err(|m| TypeError {
                            message: m,
                            span: p.span.clone(),
                        })?;
                    let sp = self.bind_pattern(&pats[0], &apply_subst_type(&s, &tok), env)?;
                    Ok(compose_subst(&s, &sp))
                } else if n == "Err" && pats.len() == 1 {
                    let (tok, terr) = (self.new_var(), self.new_var());
                    let s = self
                        .unify(tt, &Type::Result(Box::new(tok), Box::new(terr.clone())))
                        .map_err(|m| TypeError {
                            message: m,
                            span: p.span.clone(),
                        })?;
                    let sp = self.bind_pattern(&pats[0], &apply_subst_type(&s, &terr), env)?;
                    Ok(compose_subst(&s, &sp))
                } else {
                    for ed in env.enums.values().cloned() {
    ...
    fn check_matrix(&self, matrix: &[Vec<PatRef>], types: &[&Type]) -> Result<(), String> {
        ...
        match ft {
            Type::Bool => {
                self.check_constructor_matrix(matrix, "true", 0, &[], rt)?;
                self.check_constructor_matrix(matrix, "false", 0, &[], rt)?;
                Ok(())
            }
            Type::Result(ok, err) => {
                self.check_constructor_matrix(matrix, "Ok", 1, &[&**ok], rt)?;
                self.check_constructor_matrix(matrix, "Err", 1, &[&**err], rt)?;
                Ok(())
            }
            Type::Record(fields) => {
            ...
            Type::Unit => {
                self.check_constructor_matrix(matrix, "()", 0, &[], rt)?;
                Ok(())
            }
            Type::UserDefined(name, args) => {
                if let Some(ed) = self.env.get_enum(name).cloned() {
                    let mut subst = HashMap::new();
                    ...
                    for v in &ed.variants {
                        let ats: Vec<Type> = v
                            .fields
                            .iter()
                            .map(|f| apply_subst_type(&subst, f))
                            .collect();
                    ...
                    self.check_wildcard_matrix(matrix, rt)
                }
            }
    ...
    fn check_wildcard_matrix(&self, matrix: &[Vec<PatRef>], rt: &[&Type]) -> Result<(), String> {
        ...
        for row in matrix {
            match row[0].node() {
                Pattern::Wildcard | Pattern::Variable(..) => nm.push(row[1..].to_vec()),
                _ => {}
            }
        }
        if nm.is_empty() {
    ...
    fn check_constructor_matrix(
                ...
                Pattern::Constructor(c, args) => {
                    if c == ctor {
                        let mut nr: Vec<PatRef> =
                            args.iter().map(|a| PatRef::Original(a)).collect();
        ...
        self.check_matrix(&nm, &nt)
            .map_err(|_| format!("Missing {}", ctor))
    }
        ...
        match (t1, t2) {
            ...
            (Type::Arrow(p1, r1, e1), Type::Arrow(p2, r2, e2)) => {
                ...
                for ((n1, t1), (n2, t2)) in p1.iter().zip(p2) {
                    if n1 != n2 {
                        return Err(format!("Label mismatch: {} vs {}", n1, n2));
                    }
            ...
            (Type::Result(o1, er1), Type::Result(o2, er2)) => {
                let s1 = self.unify(o1, o2)?;
                let s2 = self.unify(&apply_subst_type(&s1, er1), &apply_subst_type(&s1, er2))?;
...
pub fn apply_subst_type(subst: &Subst, typ: &Type) -> Type {
    match typ {
        ...
        Type::Result(o, e) => Type::Result(
            Box::new(apply_subst_type(subst, o)),
            Box::new(apply_subst_type(subst, e)),
        ),
...
fn get_free_vars_type(typ: &Type) -> HashSet<String> {
    match typ {
        ...
        Type::Result(o, e) => {
            let mut s = get_free_vars_type(o);
            s.extend(get_free_vars_type(e));
            s
        }
        Type::Ref(i) | Type::Linear(i) | Type::Borrow(i) | Type::List(i) | Type::Array(i) => {
