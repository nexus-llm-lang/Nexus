...
#
# Why a cache and not mktemp: the default compiler-payload path `exec`s
# wasmtime, which REPLACES this shell process — so the `cleanup` EXIT trap
# never fires and a per-run `mktemp` extraction is leaked. Accumulated over a
# dev/test session this exhausted the /tmp tmpfs. A content-addressed cache
# extracts each payload version exactly once, reuses it on every subsequent run
# (no extraction, no leak), and self-deduplicates (one file per payload sha).
#
