...
| `"std:jsonrpc"` | pure + `Console` | JSON-RPC 2.0 base-protocol framing — `frame_message`/`unframe_one` (pure), `read_message`/`write_message`/`dispatch_step` (I/O), plus `IncomingMessage` classifier and standard error builders |
