...
"#;
    assert!(
        check(src).is_ok(),
        "Env port with PermEnv should typecheck: {:?}",
        check(src).err()
    );
}
...
"#;
    assert!(check(src).is_ok(), "Env.set with PermEnv should typecheck");
}
...
"#;
    assert_eq!(run(src).unwrap(), Value::String("mock_value".to_string()));
}
