...
fn codegen_minimal_wasm_output() {
    exec_with_stdlib(&read_fixture("test_codegen_minimal.nx"));
}

#[test]
fn codegen_validate_wasm_output() {
    // Run the Nexus fixture that generates 5 WASM files from LIR programs
    exec_with_stdlib(&read_fixture("test_codegen_validate.nx"));

    // Validate each generated file is structurally valid WASM
    let files = [
        "nxc_test_empty.wasm",
        "nxc_test_i64.wasm",
        "nxc_test_call.wasm",
    ...
    for path in &files {
        ...
        wasmparser::Validator::new().validate_all(&bytes).unwrap_or_else(|e| {
            panic!("{} failed validation: {}", path, e);
        });
    }

    // Execute the simpler modules and verify results
    let engine = wasmtime::Engine::default();

    // empty main: should run without error
    {
    ...
    // if statement: should run without error (returns unit)
    {
